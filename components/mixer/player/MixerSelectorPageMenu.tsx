  ...
  return (
    <div className="flex min-w-0 items-center justify-between">
      <div onClick={() => onChange(-1)} className="-m-16 shrink-0 cursor-pointer p-16">
        <Icon type="left" className="w-16 text-white/30" />
      </div>
      <div className="flex min-w-0 grow flex-col gap-4">
        <div className="truncate text-center text-16 font-500 text-white/30">{date}</div>
        <div className="truncate text-center text-18 font-600 text-white">{title}</div>
      </div>
      <div onClick={() => onChange(1)} className="-m-16 shrink-0 cursor-pointer p-16">
        <Icon type="right" className="w-16 text-white/30" />
      </div>
    </div>
  );
}
